...
fn node_new(min_value: u32, max_value: u32, mapped_bitmask: u32, partially_mapped_bitmask: u32) -> u32 {
    var node = 0;
    insertBits(node, min_value, NODE_MIN_OFFSET, NODE_MIN_COUNT);
    insertBits(node, max_value, NODE_MAX_OFFSET, NODE_MAX_COUNT);
    insertBits(node, mapped_bitmask, MAPPED_RESOLUTIONS_OFFSET, MAPPED_RESOLUTIONS_COUNT);
    insertBits(node, partially_mapped_bitmask, PARTIALLY_MAPPED_RESOLUTIONS_OFFSET, PARTIALLY_MAPPED_RESOLUTIONS_COUNT);
    return node;
}
...
fn node_set_min(node: u32, new_min: u32) {
    insertBits(node, new_min, NODE_MIN_OFFSET, NODE_MIN_COUNT);
}
...
fn node_set_max(node: u32, new_max: u32) {
    insertBits(node, new_max, NODE_MAX_OFFSET, NODE_MAX_COUNT);
}
...
fn node_set_mapped_resolutions(node: u32, new_mapped_resolutions_bitmask: u32) {
    insertBits(node, new_mapped_resolutions_bitmask, MAPPED_RESOLUTIONS_OFFSET, MAPPED_RESOLUTIONS_COUNT);
}
...
fn node_set_partially_mapped_resolutions(node: u32, new_partially_mapped_resolutions_bitmask: u32) {
    insertBits(node, new_partially_mapped_resolutions_bitmask, PARTIALLY_MAPPED_RESOLUTIONS_OFFSET, PARTIALLY_MAPPED_RESOLUTIONS_COUNT);
}
