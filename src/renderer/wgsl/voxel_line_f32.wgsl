...
fn create_voxel_line(ray: Ray, t_entry: f32, t_exit: f32, page_table: ptr<function, PageTableMeta, read_write>) -> VoxelLine {
    ...
    let inverse_brick_scale = 1. / float3(pt.page_table_extent);
    ...
    let t_delta = inverse_brick_scale / abs(r.direction);
    ...
    let next_axis_crossing = float3(next_axis_crossing_index) * inverse_brick_scale;
    ...
    let state = TraversalState(
        // entry and exit will be set further down by calling _update_entry_and_exit
        float3(),
        float3(),
        current_brick,
        0., // t_entry
